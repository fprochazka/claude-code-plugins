...
- **Report:** the events the parent wants to hear about, with thresholds where they apply ("quiet 2m after green", "idle 3h"). Everything else you observe goes into the ledger and the scorecard line only.
...
- **Markers:** the parent's own comment marker (`<!-- sdlc:mr-babysit -->` or `<!-- code-review:watch -->`) and the other side's. A note whose last line equals the parent's marker is the parent's own; the other side's marker names the other command; any other `<!-- x:y -->` last line is some other automation. Last-line equality, never a substring match.
...
- **Ignore markers, optional:** comment markers whose threads are scaffolding, not review, so a note under them is never a `NOTES_CHANGED` event.
...
1. **First call, one-shot.** Run `${CLAUDE_PLUGIN_ROOT}/scripts/mr-state.py --mr-url <url> ...` without the wait flag, with `--reset` when your ledger file does not exist yet (a new watch) and without it when it does (you are a replacement watcher continuing an old state). Whatever the exit code, send one scorecard-only message with zero events, so the parent knows what it is starting from. Stale changes an earlier reader left in the state are not events.
2. **Every later call, wait mode:** the same command with `--wait-for-state-change-timeout-minutes 8` (and `--poll-interval-seconds 300` in the waiting cadence), with a Bash tool timeout of 10 minutes. The script keeps its own state on disk, so a change that lands while you are between calls is caught on the first probe of the next call.
3. **After every return, whatever the exit code:** if the ticket block is present, read the ticket state once and treat a change as an event; if 30 minutes have passed since your last message, send the heartbeat. Then:
   - **Exit 0, a change:** read the change lines. For each, decide whether the parent subscribed to it; classify it (below); send one message per run covering every subscribed event, with the details the parent needs to act.
   - **Exit 3, timeout:** nothing moved on the MRs.
   - **Exit 5, every probe failed:** relay the `READ_FAILED` lines and keep going; the host or the token is the likely cause, and the parent decides.
   Go to step 2.
...
**Ids you add on top of the script's:** `HANDSHAKE`, `TICKET_CHANGED`, `VERDICT_CHANGED`, `VERDICT_STALE`, `QUIET`, `IDLE`, `HEARTBEAT`. **Messages that go out regardless of the subscription:** the first scorecard, `STATE_CHANGED` to merged or closed, `TICKET_CHANGED` to a terminal state, `IDLE`, `HEARTBEAT`, and `DEPENDENCY MISSING`.

Suppress two known false positives: a thread that reads unresolved right after a force-push, which is a re-do the parent handles; and threads under the parent's ignore markers, which are scaffolding, not review.
...
The heartbeat, id `HEARTBEAT`, goes every 30 minutes even when nothing moved, checked after every script return, not only on a timeout: the scorecard lines, how long since the last movement, and what you are waiting on. A parent that hears nothing for 45 minutes assumes you are dead.

## The ledger

Rewrite your ledger after every run, before you send. It is what a replacement watcher reads to continue, so it holds what you cannot re-derive: the MR set with worktree paths, the ticket block, the markers, the subscription and cadence, the last handshake reading you reported, the reviewer profiles you derived, the parent's acks, the timestamp of the last movement you observed, the timestamp and first line of your last message, the events you have already reported (so a level that holds, like "behind by 4", is not re-sent every run), and your own agent id. UTC timestamps with a `Z` suffix. The script's own state under `<tmp>/glab-state/` is not the ledger; do not copy it.
