...
- **A full read** (no wait flag) fetches everything: `--all` is the default, `--comments` and `--pipeline` narrow it. A pipeline that did not move since the last dump is not dumped again, since that fetch costs seconds; the existing dump is named and `--pipeline` forces a fresh one. The first read of an MR establishes a baseline and says so; every later read prints the diff since the previous run.
- **The wait mode** probes every minute and exits the moment a change appears or the timeout passes; the `result:` line at the end says which. Keep the timeout under your Bash tool's timeout, at most 8 minutes, since the script does not start a probe it could not finish inside the window. On a change it fetches only what the change needs: the discussion dump when a note moved, the pipeline dump when the pipeline moved. A probe is two API calls per MR, the MR object and the newest note, plus approvals when `updated_at` moved and the project's default branch once, so a set of five MRs costs about ten calls a minute.
...
| `READ_FAILED` | one MR could not be read this probe; the others were. The script keeps going, and exits non-zero (`5`) only when nothing could be read at all |
