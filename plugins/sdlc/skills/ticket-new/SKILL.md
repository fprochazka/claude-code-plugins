...
Spawn one subagent to resolve the identifiers and create the ticket. Its prompt must open with a skill-load instruction naming the installed skill that covers the tracker resolved by `teamwork:workflow-identify`: `First, invoke the <skill-name> skill to load its usage guidance before running any commands.`
